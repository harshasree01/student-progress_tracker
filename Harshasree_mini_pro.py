{
  "cells": [
    {
      "cell_type": "code",
      "source": [
        "# STUDENT PROGRESS TRACKER SYSTEM\n",
        "\n",
        "def students_data():\n",
        "    marks = []\n",
        "\n",
        "    # Asking user for personal details\n",
        "    name = input(\"Enter Student Name: \")\n",
        "    student_class = input(\"Enter Student Class: \")\n",
        "    rollno = input(\"Enter Student Roll Number: \")\n",
        "\n",
        "    # Taking subjects count\n",
        "    subs = int(input(\"\\nHow many subjects do you want to enter? \"))\n",
        "    totalmarks = 0\n",
        "    print()\n",
        "\n",
        "    # Taking marks\n",
        "    for i in range(subs):\n",
        "        mark = int(input(f\"Enter marks for subject {i+1}: \"))\n",
        "        marks.append(mark)\n",
        "        totalmarks += mark\n",
        "\n",
        "    # Calculations\n",
        "    average = totalmarks / subs\n",
        "    highest = max(marks)\n",
        "    lowest = min(marks)\n",
        "    percentage = (totalmarks / (subs * 100)) * 100\n",
        "\n",
        "    # Grade Calculation\n",
        "    if average >= 90:\n",
        "        grade = \"A\"\n",
        "    elif average >= 75:\n",
        "        grade = \"B\"\n",
        "    elif average >= 50:\n",
        "        grade = \"C\"\n",
        "    elif average >= 35:\n",
        "        grade = \"D\"\n",
        "    else:\n",
        "        grade = \"Fail\"\n",
        "\n",
        "    # Pass/Fail Logic\n",
        "    status = \"Pass\"\n",
        "    for m in marks:\n",
        "        if m < 35:\n",
        "            status = \"Fail\"\n",
        "            grade = \"Fail\"\n",
        "            break\n",
        "\n",
        "    # Remarks\n",
        "    if grade == \"A\":\n",
        "        remarks = \"Excellent\"\n",
        "    elif grade == \"B\":\n",
        "        remarks = \"Good\"\n",
        "    elif grade == \"C\":\n",
        "        remarks = \"Average\"\n",
        "    elif grade == \"D\":\n",
        "        remarks = \"Needs Improvement\"\n",
        "    else:\n",
        "        remarks = \"Needs Improvement\"\n",
        "\n",
        "    print(\"\\n\")\n",
        "    return name, student_class, rollno, marks, totalmarks, average, highest, lowest, percentage, grade, status, remarks\n",
        "\n",
        "\n",
        "# MAIN PROGRAM\n",
        "while True:\n",
        "    print(\"\\n-------------------------------------------\")\n",
        "    print(\"        STUDENT PROGRESS TRACKER SYSTEM\")\n",
        "    print(\"-------------------------------------------\\n\")\n",
        "\n",
        "    # Get student details + marks + results\n",
        "    (\n",
        "        name, student_class, rollno, marks, totalmarks,\n",
        "        average, highest, lowest, percentage, grade,\n",
        "        status, remarks\n",
        "    ) = students_data()\n",
        "\n",
        "    # Display report\n",
        "    print(\"\\n---------- STUDENT REPORT -----------\")\n",
        "    print(f\"Name            : {name}\")\n",
        "    print(f\"Class           : {student_class.upper()}\")\n",
        "    print(f\"Roll Number     : {rollno}\")\n",
        "    print(f\"Highest Mark    : {highest}\")\n",
        "    print(f\"Lowest Mark     : {lowest}\")\n",
        "    print(f\"Total Marks     : {totalmarks}\")\n",
        "    print(f\"Average Marks   : {average:.2f}\")\n",
        "    print(f\"Percentage      : {percentage:.2f}%\")\n",
        "    print(f\"Grade           : {grade}\")\n",
        "    print(f\"Status          : {status}\")\n",
        "    print(f\"Remarks         : {remarks}\")\n",
        "    print(\"-------------------------------------\\n\")\n",
        "\n",
        "    # Menu\n",
        "    print(\"What do you want to do next?\\n\")\n",
        "    print(\"1. Enter marks again\")\n",
        "    print(\"2. View report again\")\n",
        "    print(\"3. Exit\")\n",
        "\n",
        "    choice = input(\"\\nEnter your choice: \")\n",
        "\n",
        "    if choice == \"1\":\n",
        "        continue\n",
        "    elif choice == \"2\":\n",
        "        continue\n",
        "    elif choice == \"3\":\n",
        "        print(\"Exiting... Thank you!\")\n",
        "        break\n",
        "    else:\n",
        "        print(\"Invalid Option! Try again.\")"
      ],
      "metadata": {
        "id": "2xRAW__b25Tp",
        "outputId": "0a71250d-8ddf-4757-86eb-3a8c91fe26cd",
        "colab": {
          "base_uri": "https://localhost:8080/"
        }
      },
      "execution_count": null,
      "outputs": [
        {
          "output_type": "stream",
          "name": "stdout",
          "text": [
            "\n",
            "-------------------------------------------\n",
            "        STUDENT PROGRESS TRACKER SYSTEM\n",
            "-------------------------------------------\n",
            "\n",
            "Enter Student Name: harshasree\n",
            "Enter Student Class: btech 2nd year\n",
            "Enter Student Roll Number: 25B25A4510\n",
            "\n",
            "How many subjects do you want to enter? 5\n",
            "\n",
            "Enter marks for subject 1: 90\n",
            "Enter marks for subject 2: 100\n",
            "Enter marks for subject 3: 80\n",
            "Enter marks for subject 4: 70\n",
            "Enter marks for subject 5: 75\n",
            "\n",
            "\n",
            "\n",
            "---------- STUDENT REPORT -----------\n",
            "Name            : harshasree\n",
            "Class           : BTECH 2ND YEAR\n",
            "Roll Number     : 25B25A4510\n",
            "Highest Mark    : 100\n",
            "Lowest Mark     : 70\n",
            "Total Marks     : 415\n",
            "Average Marks   : 83.00\n",
            "Percentage      : 83.00%\n",
            "Grade           : B\n",
            "Status          : Pass\n",
            "Remarks         : Good\n",
            "-------------------------------------\n",
            "\n",
            "What do you want to do next?\n",
            "\n",
            "1. Enter marks again\n",
            "2. View report again\n",
            "3. Exit\n",
            "\n",
            "Enter your choice: \n",
            "Invalid Option! Try again.\n",
            "\n",
            "-------------------------------------------\n",
            "        STUDENT PROGRESS TRACKER SYSTEM\n",
            "-------------------------------------------\n",
            "\n",
            "Enter Student Name: harshasree\n",
            "Enter Student Class: btech 1st year\n",
            "Enter Student Roll Number: 24B24A4510\n",
            "\n",
            "How many subjects do you want to enter? 6\n",
            "\n",
            "Enter marks for subject 1: 100\n",
            "Enter marks for subject 2: 98\n",
            "Enter marks for subject 3: 100\n",
            "Enter marks for subject 4: 78\n",
            "Enter marks for subject 5: 87\n",
            "Enter marks for subject 6: 90\n",
            "\n",
            "\n",
            "\n",
            "---------- STUDENT REPORT -----------\n",
            "Name            : harshasree\n",
            "Class           : BTECH 1ST YEAR\n",
            "Roll Number     : 24B24A4510\n",
            "Highest Mark    : 100\n",
            "Lowest Mark     : 78\n",
            "Total Marks     : 553\n",
            "Average Marks   : 92.17\n",
            "Percentage      : 92.17%\n",
            "Grade           : A\n",
            "Status          : Pass\n",
            "Remarks         : Excellent\n",
            "-------------------------------------\n",
            "\n",
            "What do you want to do next?\n",
            "\n",
            "1. Enter marks again\n",
            "2. View report again\n",
            "3. Exit\n",
            "\n",
            "Enter your choice: 3\n",
            "Exiting... Thank you!\n"
          ]
        }
      ]
    }
  ],
  "metadata": {
    "colab": {
      "provenance": []
    },
    "kernelspec": {
      "display_name": "Python 3",
      "name": "python3"
    }
  },
  "nbformat": 4,
  "nbformat_minor": 0
}
